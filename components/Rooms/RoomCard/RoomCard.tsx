...
const RoomCard = ({ image, name, price, addToCart, id, cart }: RoomCardProps) => {
  ...
  return (
    <div onClick={addToCart} title={check() ? "Room is already in cart": ""} id={`${id}`} style={check() ? { filter: "grayscale(.7)", cursor: "not-allowed"} : {filter: "grayscale(0)", cursor: "pointer"}} className={s.RoomCard}>
      <Image src={image} alt="" />
      <div>
        <h3>{name}</h3>
        ...
        {!check() ? <button style={check() ? { filter: "grayscale(1)", cursor: "not-allowed"} : {filter: "grayscale(0)", cursor: "pointer"}} onClick={addToCart}> Book this room</button> : <strong><small> Room has been booked</small> </strong> }
